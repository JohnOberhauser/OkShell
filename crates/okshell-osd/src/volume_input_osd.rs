...
#[derive(Debug)]
pub struct VolumeInputOsdModel {
...
#[derive(Debug)]
pub enum VolumeInputOsdOutput {}

pub struct VolumeInputOsdInit {
    pub monitor: gdk::Monitor,
}

#[derive(Debug)]
pub enum VolumeInputOsdCommandOutput {
    DeviceChanged,
    VolumeChanged,
    Hide,
}
...
#[relm4::component(pub)]
impl Component for VolumeInputOsdModel {
    type CommandOutput = VolumeInputOsdCommandOutput;
    type Input = VolumeInputOsdInput;
    type Output = VolumeInputOsdOutput;
    type Init = VolumeInputOsdInit;
    ...
    fn init(
        ...
        spawn_default_input_watcher(&sender, None, || VolumeInputOsdCommandOutput::DeviceChanged);

        let model = VolumeInputOsdModel {
            active_device_watcher_token: WatcherToken::new(),
            hide_token: WatcherToken::new(),
            icon_name: "microphone-sensitivity-medium-symbolic".to_string(),
            slider_value: 0.0,
            shown_count: 0,
        };
    ...
    fn update_with_view(
        ...
        match message {
            VolumeInputOsdInput::UpdateDevice(device) => {
                    ...
                    shutdown
                        .register(async move {
                            ...
                            if !token.is_cancelled() {
                                out.send(VolumeInputOsdCommandOutput::Hide).ok();
                            }
                        })
                        .drop_on_shutdown()
                });
            }
            VolumeInputOsdInput::Show => {
    ...
    fn update_cmd(
        ...
        match message {
            VolumeInputOsdCommandOutput::DeviceChanged => {
                let default_input = audio_service().default_input.get();
                if let Some(audio_device) = default_input {
                    sender.input(VolumeInputOsdInput::UpdateDevice(audio_device.clone()));

                    let token = self.active_device_watcher_token.reset();

                    spawn_input_device_volume_mute_watcher(&audio_device, token, &sender, || {
                        VolumeInputOsdCommandOutput::VolumeChanged
                    });
                }
            }
            VolumeInputOsdCommandOutput::VolumeChanged => {
                if let Some(default_input) = audio_service().default_input.get() {
                    sender.input(VolumeInputOsdInput::UpdateDevice(default_input));
                    sender.input(VolumeInputOsdInput::Show);
                }
            }
            VolumeInputOsdCommandOutput::Hide => {
                sender.input(VolumeInputOsdInput::Hide);
            }
        }
    }
}
