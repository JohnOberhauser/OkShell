...
use okshell_image::lut::{apply_theme_filter, embedded_clut, rgba_to_texture, snapshot_and_recolor};
...
pub fn set_icon(
    ...
    if let Some(themed) = icon.downcast_ref::<gio::ThemedIcon>() {
        ...
        if icon_theme.has_icon(name_strs[0]) {
            ...
            set_paintable_recolored(image, &paintable, color_theme, should_recolor);
            return;
        }
    }
...
/// Set a paintable on the image, optionally recoloring it through the CLUT pipeline.
fn set_paintable_recolored(
    image: &gtk::Image,
    ...
    // Try file-backed path first
    if let Some(file) = paintable.file() {
        if let Some(path) = file.path() {
            let cancel = AtomicBool::new(false);
            if let Some(result) = apply_theme_filter(&path, color_theme, 1.0, &cancel) {
                if let Some(texture) = rgba_to_texture(&result.buf, result.width, result.height) {
    ...
    // Fallback: snapshot-render the paintable, recolor the pixels
    if let Some(texture) = snapshot_and_recolor(paintable, color_theme) {
        image.set_paintable(Some(&texture));
        return;
    }
...
fn has_usable_icon(icon: &gio::Icon) -> bool {
    if let Some(themed) = icon.downcast_ref::<gio::ThemedIcon>() {
        ...
        themed
            .names()
            .iter()
            .any(|n| default_theme.has_icon(n.as_str()))
...
fn find_icon_from_path(icon_theme: &gtk::IconTheme, file: &gio::File) -> Option<String> {
    let path = file.path()?;
