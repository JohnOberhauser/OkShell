//! Image processing utilities for okshell.
//!
//! This crate provides palette remapping for wallpapers using lutgen's
//! Gaussian RBF interpolation via Hald CLUTs. Static theme CLUTs are
//! precomputed by `xtask-generate-cluts` and checked into `cluts/`.
//!
//! For dynamic themes (Default/Wallpaper), CLUTs can be generated at runtime.
...
// ── Embedded CLUTs (generated by xtask-generate-cluts) ──────────────────────
...
/// Configuration for runtime palette remap (dynamic themes only).
#[derive(Debug, Clone)]
pub struct PaletteRemapConfig {
    pub hald_level: u8,
    pub shape: f64,
    pub nearest: usize,
    pub lum_factor: f64,
...
impl Default for PaletteRemapConfig {
    fn default() -> Self {
        Self {
            hald_level: 8,
            shape: 96.0,
...
/// Generate a CLUT at runtime for a dynamic palette (e.g. wallpaper-derived).
pub fn generate_runtime_clut(
    palette: &[[u8; 3]],
    config: &PaletteRemapConfig,
    ...
    remapper.par_generate_lut(config.hald_level)
}

/// Apply a runtime-generated CLUT to an image file.
pub fn apply_runtime_filter(
    path: &Path,
    palette: &[[u8; 3]],
    config: &PaletteRemapConfig,
    cancel: &AtomicBool,
    ...
    let hald_clut = generate_runtime_clut(palette, config);
...
/// Reconstruct a Hald CLUT image from embedded raw RGB bytes.
